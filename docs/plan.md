...
| Хранилище | SQLite | Чаты, галерея, библиотека моделей, настройки |
...
- [ ] Библиотека моделей: добавить файл (кнопка или перетаскивание), тип и светофор определяются сами.
- [ ] Подхват моделей из папок LM Studio, Ollama и ComfyUI без копирования.
- [ ] Каталог: подборка проверенных моделей + поиск по HuggingFace с фильтром «пойдёт на моём ПК», скачивание по ссылке.
- [ ] Автонастройка запуска: сколько слоёв на видеокарту, размер контекста, выбор квантизации под память.
- [ ] Чат: стриминг, markdown и код, остановка, перегенерация, история, поиск по чатам.
- [ ] Пресеты вместо параметров: «Точнее ↔ Креативнее», роли (помощник, переводчик, программист).
- [ ] Понятные ошибки с кнопками действий.
- [ ] Лицензия модели видна в карточке (некоторые модели запрещают коммерческое использование).
